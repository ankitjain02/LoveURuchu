...
-(void)setIconButton:(NSString *)icon withType:(NSInteger)type withColor:(UIColor*)color
{
    self.messageLabel = [[UILabel alloc] init];
    //self.imgIcon = [[UIImageView alloc] initWithImage:icon];
    [self.messageLabel setFrame:CGRectMake(0, 0, CGRectGetWidth(self.bounds)-CGRectGetWidth(self.bounds)/2, CGRectGetHeight(self.bounds)-CGRectGetHeight(self.bounds)/2)];
    [self.messageLabel setTextAlignment:NSTextAlignmentCenter];
    [self.messageLabel setText:icon];
    ...
    [self.messageLabel setTintColor:color];
    switch (type) {
        case VMMakeLocationTop:
            [self.messageLabel setCenter:CGPointMake(CGRectGetWidth(self.bounds)/2, CGRectGetHeight(self.bounds)/2 -5 )];
            break;
        case VMMakeLocationBottom:
            [self.messageLabel setCenter:CGPointMake(CGRectGetWidth(self.bounds)/2, CGRectGetHeight(self.bounds)/2 +5 )];
            break;
